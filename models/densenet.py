...
class DenseNetClassifier(nn.Module):
    def __init__(self, arch_name='densenet121', n_label=10, pretrained=False, dropout=0,
                 fine_tune_layers=1, emb_size=256, in_channels=3, **kwargs):
        super(DenseNetClassifier, self).__init__(**kwargs)
        ...
        self.hidden_layers = build_mlp(input_size, (), emb_size,
                                       dropout=dropout,
                                       use_batchnorm=False,
    ...
    def fine_tune(self, fine_tune_layers):
        ...
        for c in list(self.densenet.children())[
                 0 if fine_tune_layers < 0 else len(list(self.densenet.children())) - (1 + fine_tune_layers):]:
